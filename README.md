...
![브로셔 이미지 2025-02-05 오전 2.01.51.png](attachment:63f89302-a86d-4249-a0c1-26de53e62f68:브로셔_이미지_2025-02-05_오전_2.01.51.png)
...
- 테스트 환경
    ...
    - 테스트 조건
        ...
        - Loop Count: 3000
1. 문제상황
    - JMeter를 사용해 부하 테스트를 진행한 결과, 단일 서버 환경에서 일정 TPS를 유지하다가 특정 시점 이후 성능이 급격히 감소되며 요청이 실패
2. 원인분석
...
4. 결과
    ...
    - 서버 확장을 통해 부하를 분산시키는 것이 효과적인 해결책

![스크린샷 2025-02-07 오전 12.52.53.png](attachment:412c4fb1-781c-450a-b415-6720a3482282:스크린샷_2025-02-07_오전_12.52.53.png)

-단일 서버 (특정 시간 이후 요청 실패)

![스크린샷 2025-02-06 오후 5.19.01.png](attachment:fc0f55c4-741b-4640-ba6b-6380c586653d:스크린샷_2025-02-06_오후_5.19.01.png)
...
- **`Presigned URL`을 활용하여** 클라이언트에서 직접 S3로 업로드 가능
...
2. **`Presigned URL` 발급 후 클라이언트에서 직접 S3로 업로드**
...
- **`Presigned URL`을 활용한 보안 강화** (직접 접근 제한, 일정 시간 후 만료)
- **`ACL` 정책 및 버킷 정책을 활용한 접근 제어**
- **`CloudFront`와 연동하여 이미지 캐싱 및 빠른 응답 속도 제공**
- **`S3 Intelligent-Tiering`을 적용하여 스토리지 비용 최적화**
...
- **S3 Transfer Acceleration을 활용하여 업로드 속도 향상**

### 💥 **빠른 이미지 제공**

- **`CloudFront`와 연계하여 전 세계 어디서든 빠른 이미지 로딩 제공**
- **`Presigned URL`을 활용하여 안전하고 효율적인 업로드 방식 구현**
- **`Edge Location`을 활용한 이미지 서빙 최적화**

### 💥 **보안 강화**

- **S3 버킷 정책 및 IAM 역할을 설정하여 외부 접근 차단**
- **`Signed URL`을 활용하여 인증된 사용자만 접근 가능하도록 구성**
...
### **🖤 동시간 다중 채팅방 시스템 (WebSocket & STOMP & SockJS)**
...
- Java 버전은 무엇을 사용해야 할까요?
    ...
    | **장점** | • `switch` 패턴 매칭 (Preview)
    • `record` 클래스 도입
    • `"""`을 사용한 텍스트 블록 기능 제공 | **• 가상 스레드(Virtual Threads) 도입**
    - OS 스레드 사용량을 줄이고, 수백만 개의 가상 스레드를 실행 가능
    - 기존 Java 코드 수정 없이 사용 가능
    **• 기본 문자 인코딩이 UTF-8로 변경** |
    ...
    **Java 21을 선택** ☑️
    ...
    Java 21은 **더 긴 LTS 지원(2031년까지)과 성능 최적화 기능**을 포함하고 있으며, 특히 **가상 스레드**는 대규모 트래픽을 처리하는 환경에서 큰 이점을 제공합니다. 
    
    결론적으로, **Java 17 대비 성능과 유지보수성이 뛰어나므로 Java 21을 채택하는 것이 적절합니**다. 
...
- 인증 인가는 어떤 방식으로 해야할까요?
    ...
    현재 프로젝트의 **인증(Authentication) 방식**을 결정하기 위해, **JWT(Json Web Token) 세션(Session) 기반 인증**을 비교하였습니다.
    ...
    | --- | --- | --- |
    | **개념** | 클라이언트가 로그인하면 서버에서 JWT를 생성하여 클라이언트에 전달.
    이후 요청 시 JWT를 포함하여 인증 진행 | 클라이언트가 로그인하면 서버에서 세션을 생성하고, 세션 ID를 클라이언트에 전달. 이후 요청 시 세션 ID를 포함하여 인증 진행 |
    | **보관 방식** | 클라이언트(브라우저, 모바일 앱 등)에서 JWT를 **로컬 스토리지 또는 쿠키에 저장** | 서버에서 세션 정보를 저장하며, 클라이언트는 세션 ID를 쿠키에 저장 |
    | **확장성** | 서버가 상태를 저장하지 않기 때문에 **확장성이 뛰어남 (Stateless)** | 서버에서 세션을 관리하므로 **서버 부하 증가 가능** |
    | **보안** | 토큰이 유출되면 누구나 사용 가능 → **짧은 유효기간 + Refresh Token 필요** | 세션 탈취 위험 존재 → **HTTPS 및 보안 정책 강화 필요** |
    | **장점** | ☑️ 서버 부하 없음 (Stateless)
    ☑️ 여러 서비스 (API)에서 쉽게 공유 가능
    ☑️ OAuth 같은 인증 시스템과 연동 용이 | ☑️ 세션이 서버에서 관리되므로 보안성이 더 뛰어남 
    ☑️ 토큰이 탈취되었을 때 서버에서 강제 만료 가능 |
    | **단점** | ❌ 유출 시 즉시 만료가 어렵고 Refresh Token이 필요함
    ❌ 토큰 길이가 길어 요청 크기 증가 | ❌ 서버에 부하 발생 (Stateful)
    ❌ 세션을 서버에서 유지해야 하므로 확장성 낮음 |
    
    ---
    
    ## 4. JWT를 선택(Stateless 인증 방식)
    
    ☑️ **확장성이 중요한 서비스**이므로, **상태를 저장하지 않는 방식(JWT)**이 적합함
    ...
    ☑️ **Stateless 방식으로 서버 부담이 적음** → API 서버 확장에 유리
    
    ☑️ **마이크로서비스 및 OAuth 연동에 적합**
    
    ☑️ **토큰 기반 인증 방식으로 클라이언트 간 공유 가능**
    
    ☑️ **세션 관리가 필요하지 않아 유지보수가 용이**
    ...
    2. **보안 강화**
        - JWT를 **HttpOnly & Secure 쿠키에 저장**하여 XSS 공격 방지
        - HTTPS를 강제하여 네트워크에서 토큰이 유출되지 않도록 보호
    3. **로그아웃 및 강제 만료 처리**
        - Redis 등을 활용하여 **Blacklist 기능**을 추가하여 특정 JWT 차단
    4. **JWT 기반 인증을 Spring Security와 연동**
        - Spring Security + JWT 조합으로 인증 필터 구현
    ...
    특히 **OAuth 2.0 및 마이크로서비스 아키텍처와의 연동이 쉬워**, 장기적으로 유지보수성과 확장성을 고려했을 때 최적의 선택입니다. 
...
- DB는 어떤 것을 사용해야 할까요?
    ...
    **읽기 성능, 확장성, 복잡한 쿼리 처리, 대용량 데이터 관리, 동시성 처리** 등의 요소를 고려하여 최적의 DBMS를 선택했습니다.
    ...
    | **아키텍처** | **멀티 스레드 아키텍처**  - 한 프로세스가 여러 스레드를 가짐  - **높은 메모리 효율성** | **멀티 프로세스 아키텍처**  - 각 프로세스가 독립적인 메모리 공간을 가짐  - **더 높은 안정성과 확장성** |
    | **성능** | **읽기 작업에 최적화** | **복잡한 쿼리, 대용량 데이터 처리, 쓰기 작업에 최적화** |
    | **사용 사례** | **웹 애플리케이션, 가벼운 서비스** | **대규모 애플리케이션, 데이터 웨어하우스, 복잡한 시스템** |
    | **장점** | ☑️ 읽기 작업에 최적화되어 단순한 쿼리 처리 시 PostgreSQL보다 **뛰어난 성능**  ☑️ **사용 편의성**이 높음
    ☑️ 역사가 깊고 사용층이 넓어 **자료와 커뮤니티가 풍부** | ☑️ OpenAI API의 **JSON 응답 데이터 저장** 및 검색 최적화
    ☑️ **실시간 채팅 메시지 저장 시 동시성 처리 우수** (MVCC 활용)
    ☑️ **사용자 취향 데이터 저장 및 복잡한 쿼리 지원** (콜렉션, JSONB 지원) |
    ...
    - 대용량 데이터를 다룰 경우 추가적인 **캐싱(Redis) 활용을 고려**하여 성능을 보완 가능
    ...
    - **긍정적 영향**
        - **빠른 읽기 성능 제공** → 웹 애플리케이션의 속도 향상
        - **사용 편의성이 높아 개발 속도 증가**
        - **넓은 커뮤니티 및 풍부한 문서 지원**
    - **부정적 영향**
        - **대량 데이터의 복잡한 쿼리 처리 성능은 PostgreSQL보다 낮음**
        - **실시간 데이터 저장 시 동시성 문제가 발생할 가능성 있음** → 해결책: Redis 캐싱 및 샤딩 적용
    ...
    MySQL은 **빠른 읽기 성능과 쉬운 사용성**을 제공하며, 웹 애플리케이션 개발에 적합한 DBMS입니다.
...
- 프로젝트에 적합한 `Architecture` 는 무엇일까요?
    ...
    **"클린 아키텍처 + 레이어드 아키텍처"의 조합**이 현재 프로젝트의 요구사항과 가장 부합한다고 판단하였습니다.
...
- 채팅 기능 구현 방식이 너무 많은데, 어떤게 제일 적합할까요?
    
    ## 1. 배경 (Background)
    
    **다중 사용자 채팅 기능**을 구현을 위해, **빠른 응답 속도, 안정성, 확장성**을 고려한 적절한 기술 스택을 선정하고자 노력하였습니다.
    
    다양한 메시징 시스템(WebSocket, RabbitMQ, Kafka 등)이 존재하지만, **즉각적인 메시지 전달 및 확장성**이 중요한 채팅 서비스 특성을 반영하여 최적의 방식을 아래와 같이 결정하였습니다.
    ...
    | **단점** | ❌ WebSocket이 끊어질 경우 재연결 필요 | ❌ 즉각적인 메시지 전달이 어려움 (큐 대기 시간) | ❌ 높은 지연 시간 (채팅 서비스에는 적합하지 않음) |
    
    ---
    
    ## 4. 결정 (Decision)
    
    ☑️ **WebSocket + STOMP + SockJS + Redis Pub/Sub을 선택**
    
    - **WebSocket**을 통해 **양방향 실시간 통신** 지원
    - **STOMP 프로토콜**을 사용하여 **Pub/Sub 기반 브로드캐스트 가능**
    - **SockJS**를 적용하여 **WebSocket 미지원 환경에서도 대응 가능**
    - **Redis Pub/Sub**을 활용하여 **여러 대의 서버에서도 메시지 동기화 보장**
    
    ---
    
    ## 5. 근거 (Justification)
    
    ☑️ **즉각적인 응답**이 중요한 채팅 서비스에 최적화된 방식
    
    ☑️ **WebSocket을 활용하여 클라이언트와 서버 간 실시간 양방향 통신 지원**
    
    ☑️ **Redis Pub/Sub을 사용하여 서버 간 메시지 동기화 및 확장성 확보**
    
    ☑️ **RabbitMQ/Kafka와 비교했을 때 불필요한 오버헤드 제거**
    ...
    - **긍정적 영향**
        ...
        - **확장성 보장** → 여러 대의 서버에서 채팅 동기화 가능
        - **가벼운 프로토콜 사용** → 메시지 처리 비용 최소화
    - **부정적 영향**
        - **WebSocket이 끊어질 경우 재연결 로직 필요**
        - **Redis Pub/Sub은 메시지 내구성을 보장하지 않음** → 메시지 저장이 필요하면 DB 연동 필요
    ...
    1. **WebSocket + STOMP 프로토콜 적용**
        - 클라이언트가 WebSocket을 통해 STOMP 메시지를 주고받도록 구현
    2. **SockJS를 활용한 대체 연결 지원**
        - WebSocket 미지원 환경에서는 HTTP 기반 통신으로 대체
    3. **Redis Pub/Sub을 활용한 서버 간 동기화**
        - 여러 서버가 존재할 경우 **Redis를 통해 메시지 브로드캐스팅**
    4. **추가적인 메시지 보장을 위한 저장소 연동**
        - 채팅 메시지를 DB(MySQL, PostgreSQL 등) 또는 Redis에 저장
    
    ---
    
    ## 8. 향후 고려 사항 (Future Considerations)
    
    - **Redis 대신 Kafka로 전환 가능성 검토** (대규모 사용자 증가 시)
    - **메시지 영속성 강화를 위한 DB 연동 및 저장 전략 개선**
    - **QoS(Quality of Service) 강화** (ACK 메커니즘 추가 가능 여부 검토)
    
    ---
    
    ### ☑️ 결론: **WebSocket + STOMP + SockJS + Redis Pub/Sub 채택**
    
    WebSocket을 기반으로 **즉각적인 메시지 전달**을 보장하면서, Redis Pub/Sub을 활용하여 **서버 간 확장성 및 동기화 문제를 해결**하는 구조를 선택했습니다.
    
    RabbitMQ/Kafka 대비 **더 빠른 응답 속도와 가벼운 구조**를 유지할 수 있어 **다중 사용자 채팅 서비스에 최적화된 솔루션**입니다.
...
- AI 모델은 어떤 걸 연결할까요? 그리고 이것을 어떻게 연결해야할까요?
    
    ## 1. 배경 (Background)
    
    **사용자 맞춤 메뉴 추천**을 위한 AI 모델을 선택하기 위해, 
    
    Spring AI에서 제공하는 **연결 가능한 AI 모델(OpenAI GPT, Google Gemini 등)** 중, **성능, 비용, 확장성, 응답 속도** 등의 요소를 고려하여 적절한 모델을 결정하였습니다.
    ...
    | **단점** | ❌ GPT-4에 비해 성능이 낮음 | ❌ 응답 속도가 상대적으로 느림
    ❌ 비용 대비 가성비가 낮음 |
    ...
    - **사용자 맞춤 추천에 필요한 문맥 유지 기능이 뛰어남**
    
    ---
    
    ## 5. 근거 (Justification)
    
    ☑️ **비용 효율성이 가장 높음** → GPT-4보다 저렴하면서도 충분한 성능 제공
    
    ☑️ **Spring AI에서 공식적으로 지원하는 모델** → 연동이 간편함
    
    ☑️ **실시간 응답 최적화** → 사용자 맞춤 추천을 빠르게 제공 가능
    
    ☑️ **대량의 문맥을 처리 가능** → 이전 대화 기록을 활용하여 더 정밀한 추천 가능
    ...
    - **부정적 영향**
        - **최신 AI 모델(GPT-4, Gemini)에 비해 성능이 낮음**
        - **멀티모달 기능 부족** → 이미지 기반 추천이 필요하면 Gemini 같은 모델 고려 필요
    
    ---
    
    ## 7. 구현 전략 (Implementation Strategy)
    
    1. **Spring AI를 활용하여 OpenAI API 연동**
        - `WebClient`를 사용하여 GPT-3.5-Turbo 호출
        - Spring Security와 통합하여 API 호출 보호
    2. **비용 절감을 위한 캐싱 및 API 호출 최적화**
        - Redis를 활용하여 동일한 질문에 대한 응답 캐싱 적용
    3. **실시간 추천 시스템 최적화**
        - **이전 대화 문맥 유지** 기능 적용
        - 사용자 선호도를 반영한 메뉴 추천 로직 구현
    
    ---
    
    ## 8. 향후 고려 사항 (Future Considerations)
    
    - **GPT-4로 업그레이드 고려** (비용 절감 방안 확보 시)
    - **Gemini의 멀티모달 기능 활용 가능성 검토** (이미지+텍스트 추천 필요 시)
    - **AI 모델 Fine-tuning 및 커스텀 모델 도입 검토** (사용자 데이터 기반 최적화)
    
    ---
    
    ### ☑️ 결론: **GPT-3.5-Turbo 채택 (비용 대비 성능 최적화 고려)**
    
    **Spring AI와의 원활한 연동, 빠른 응답 속도, 가성비 높은 비용 구조**를 고려하여 **GPT-3.5-Turbo를 채택**하였습니다.
    
    GPT-4나 Gemini에 비해 성능은 낮지만, **사용자 맞춤 추천 서비스에는 충분한 성능을 제공**하며, **비용 절감 효과가 크기 때문에 최적의 선택**이라고 판단하였습니다.
...
- 캐싱을 해봅시다!
    ...
    **AI 채팅 내역 및 오픈 채팅 내역**과 **지도 검색 데이터**의 조회 성능을 최적화하기 위해 Redis를 도입하였습니다.
    ...
    MySQL만을 사용하면 **자주 조회되는 데이터의 응답 속도가 느려지고, 데이터베이스 부하가 증가**할 수 있으므로, Redis를 활용한 캐싱 전략을 적용하여 성능을 개선하였습니다.
    ...
    | **장점** | ☑️ **조회 성능 최적화** (Redis 우선 조회)
    ☑️ MySQL 부하 감소  ☑️ **자주 조회되는 데이터 캐싱에 적합** | ☑️ 데이터 일관성 유지 (DB와 Redis 동기화) | ☑️ 데이터 적재 부담 감소 (배치 처리) |
    ...
    | **장점** | ☑️ **빠른 조회 속도** (DB 조회 불필요)  ☑️ **서버 부하 감소** | ☑️ Redis 캐싱 만료 시 MySQL에서 조회 가능 |
    ...
    ☑️ **AI 채팅 내역 조회 시 Redis를 먼저 조회하여 빠른 응답 속도 제공**
    
    ☑️ **MySQL 부하 감소** → 자주 조회되는 데이터를 Redis에 저장하여 DB 쿼리 부담 완화
    
    ☑️ **지도 검색은 실시간성이 중요하므로 Redis 캐싱 전용으로 설정** → DB 저장 없이 빠른 검색 가능
    
    ☑️ **Cache-Through 방식으로 Redis 데이터 유효성 보장**
    ...
    - **긍정적 영향**
        - **AI 채팅 내역 조회 속도 향상** (Redis를 우선 조회)
        - **DB 부하 감소** (읽기 요청이 Redis에서 처리됨)
        - **지도 검색 시 초고속 조회 가능** (DB 조회 없이 인메모리 검색)
    - **부정적 영향**
        - **Redis 캐싱이 만료되면 MySQL 조회가 증가할 가능성 있음**
        - **지도 검색 데이터는 Redis가 초기화되면 다시 적재해야 함**
    
    ---
    
    ## 7. 구현 전략 (Implementation Strategy)
    
    1. **AI 채팅 및 오픈 채팅 내역 (Cache-Through 방식 적용)**
        - Redis에서 먼저 조회 (`GET chat_history:{user_id}`)
        - Redis에 데이터가 없으면 MySQL 조회 후 Redis에 저장 (`SETEX chat_history:{user_id} 3600`)
        - 일정 시간 후 Redis에서 자동 삭제 (TTL 설정)
    2. **지도 검색 데이터 (Redis 캐싱 전용, DB 저장 없음)**
        - 지도 검색 요청 시 Redis에서만 데이터 조회 (`GET location:{query}`)
        - Redis에 데이터가 없을 경우 API 또는 외부 데이터 소스에서 가져와 Redis에 저장 (`SETEX location:{query} 86400`)
        - 주기적으로 Redis 데이터 업데이트 (갱신 필요 시)
    
    ---
    
    ## 8. 향후 고려 사항 (Future Considerations)
    
    - **Redis 데이터 만료 정책 최적화** (채팅 내역 TTL 설정 조정)
    - **Redis 클러스터 도입 검토** (대규모 트래픽 대비 확장성 확보)
    - **데이터 손실 방지를 위한 백업 전략 고려** (지도 검색 데이터의 재적재 방법 개선)
    
    ---
    
    ### ☑️ 결론: **Cache-Through 방식 + Redis 캐싱 전용 방식 채택**
    
    - **AI 채팅 내역 및 오픈 채팅 내역** → **Cache-Through 방식**을 사용하여 MySQL 부하를 줄이고 빠른 조회 속도 제공합니다.
    - **지도 검색 데이터** → **Redis 캐싱 전용 (DB 저장 없음)**을 적용하여 빠른 검색 응답 보장합니다.
    - **Redis를 적극 활용하여 시스템 성능을 최적화**하고, MySQL 부하를 최소화하는 전략을 채택하였습니다.
</aside>

<aside>
💡

- EC2 배포를 할 때 서버를 분리하는 게 좋을까요?
    ...
    2. 비용 절감
       • 인스턴스 비용 절약 : 하나의 인스턴스만을 사용하면 인스턴스 임대료 절약 가능
    
    3. 빠른 통신
       • 애플리케이션 간 빠른 통신: 여러 개의 애플리케이션이 동일한 인스턴스 내에서 실행 → 애플리케이션 간 통신 속도 빠름
    
    4. 개발 및 테스트 용이
       • 통합 환경: 개발 및 테스트 환경을 단일 인스턴스에서 구축하여 개발 및 테스트 과정이 용이 | 1. 성능 향상
       • **리소스 분리**: CPU, 메모리, 네트워크 리소스를 분리하여 사용
        ⇒ 과부하 ⬇️ 성능 ⬆️
    
    **2.** 확장성
       • **독립 확장**: 각각의 서비스(메인 서버, Redis, MySQL)를 독립적으로 스케일 업/스케일 아웃 가능
    
    **3.** 고가용성 및 안정성
       • **장애 격리**: 각 서비스가 독립된 인스턴스에 배포 → 장애 발생 시 다른 서비스에 미치는 영향 최소화
       • **독립적인 업데이트 및 유지보수**: 각 서비스는 독립적으로 배포되기 때문에, 특정 컴포넌트의 업데이트나 점검 작업을 다른 컴포넌트에 영향을 주지 않고 수행
    
    **4.** 보안 강화
       • **네트워크 보안**: 각 EC2 인스턴스는 각각 다른 보안 그룹을 사용해 네트워크 접근을 제한 가능
       • **접근 제어**: 각 서버의 역할과 데이터를 분리함으로써 더 세부적인 보안 설정이 가능.
    
    **5.** 비용 최적화
       • 필요한 리소스에 따라 EC2 인스턴스 유형을 다르게 설정 가능. |
</aside>

<aside>
💡

- 로드 밸런서 사용의 장점은 무엇일까요?
    
    ### 📌 로드 밸런서 (Load Balancer)란?
    
    로드 밸런서는 여러 서버에 트래픽을 균등하게 분산하여 애플리케이션의 가용성과 성능을 최적화하는 기술입니다. 사용자의 요청을 가장 적절한 서버로 자동으로 배분하여 트래픽 병목 현상을 방지하고, 시스템의 안정성을 높이는 역할을 합니다.
    ...
    ☑️ 애플리케이션 운영 중에도 유지보수 및 업그레이드 가능
    
    ### 🔹 성능 향상
    
    ☑️ 네트워크 지연 시간을 줄여 애플리케이션 응답 속도 개선
    
    ☑️ 클라이언트 요청을 지리적으로 가까운 서버로 리디렉션하여 빠른 서비스 제공
    
    ☑️ 서버 간 로드를 균등하게 배분하여 성능 최적화
    
    ### 🔹 보안 강화
    
    ☑️ 방화벽 그룹과 연계하여 보안 계층 추가 가능
    
    ☑️ DDoS(디도스) 공격을 분산 처리하여 서비스 중단 방지
    
    ---
    
    ## 🛠 **주요 적용 기술**
    
    ☑️ **L4 로드 밸런싱** : TCP/UDP 계층에서 트래픽을 분산
    
    ☑️ **L7 로드 밸런싱** : HTTP/HTTPS 계층에서 특정 URL, 쿠키, 헤더 기반의 라우팅
    
    ☑️ **클라우드 로드 밸런서** : AWS ELB, GCP Load Balancing, Nginx, HAProxy 등
    
    로드 밸런서를 활용하면 **안정적인 서비스 운영과 성능 최적화**를 동시에 달성할 수 있습니다!
...
- CI/CD 툴은 무엇을 사용해야할까요?
    ...
    | 비용 | 툴은 무료지만 서버 유지 비용 O | - public Repository에서 무료
    - private Repository는 유료지만 무료 사용분 O |
    | 플러그인 | 1900개 이상의 플러그인 존재 | Jenkins에 비해 적음.
    스크립트로 플러그인 추가 가능. |
    | 사용성 | - GUI라 친근
    - 초기 설정이 까다로움
    - 호스팅을 직접해야함 
       → 서버 운영 및 관리 비용 발생 | - Github 사용 개발자에게 친숙
    - 초기 설정이 쉬움 |
    | 자료 | 전 세계 많은 사람들이 이용 
    → 문서가 다양 | Jenkins에 비해 문서가 적음 |
    | 총평 | - 많은 플러그인과 자료가 존재하지만 서버 설치가 필요 
    - 우리같은 소규모 프로젝트에서는 초기 세팅에 리소스가 불필요하게 투입될 수 있음 | - Github에서 사용해 친숙하며, 별도의 설치 없이 클라우드에서 관리를 하기 때문에 편리
    - deploy.yaml 파일을 작성해주는 것만으로 쉬운 파이프라인 구성 가능
    - CI/CD 의 모든 과정을 Github에서 볼 수 있어 실행 및 디버깅이 쉬움 |
</aside>

<aside>
💡

- 프론트엔드 배포툴은 무엇을 사용해야할까요?
    
    ### Vercels vs EC2+NGINX vs Cloudfront+S3 ****☑️
    
    |  | Vercles | EC2 + NGINX | Cloud Front + S3 ☑️ |
    | --- | --- | --- | --- |
    | 주요 특징 | - 간단하고 빠른 배포
    - 자동화된 CI/CD
    - Next.js, React 등 
       프레임워크 지원 | - 높은 자유도와 설정 
    가능성
    - Nginx 웹 서버를 활용한 성능 최적화
    - EC2 인스턴스 관리 필요
    - Docker로 배포환경 구축 용이 | - CDN 서비스
    - 정적 자산 전송에 특화
    - 글로벌 배포 및 캐싱
    - S3와 함께 사용 가능 (정적 웹사이트 배포에 특화되어있지만 React의 경우 내부 라우팅 기술O) |
    | 장점 | - **사용 편의성**: 개발자가 
    인프라 관리에 신경 쓸 필요 없음
    - **빠른 배포 속도**: 빌드 및 배포 과정 자동화
    - **다양한 기능**: 미리보기 
    배포, A/B 테스팅 등 지원 | - **유연성**: 서버 환경을 
    자유롭게 구성 가능
    - **성능**: Nginx를 활용한 웹 서버 최적화 가능
    - **확장성**: EC2 인스턴스 
    확장을 통해 트래픽 증가에 대응 가능
    - **비용** : EC2 프리티어 인스턴스 사용 시 비용 절감 | - **빠른 전송 속도**: CDN을 통해 전 세계 사용자에게 빠르게 콘텐츠 제공
    - **안정성**: 분산된 서버를 통해 안정적인 서비스 제공
    - **비용 효율성**: 트래픽에 따른 비용 지불
    - 백엔드 EC2 배포 시 AWS Cloud 내에서 모든 배포 가능 |
    | 단점 | - Vercel 플랫폼에 종속
    - 커스터마이징 제한
    - Web Socket 지원 X
    - 사용량에 따라 비용이 
       발생O | - 서버 구축, 설정, 관리 등 개발자 부담
    - 초기 설정 및 최적화에 
    시간과 노력이 필요
    - 서버 보안에 대한 책임이 개발자에게 있음 | - CloudFront 설정 및 S3 연동 과정이 복잡할 수 있음
    - 트래픽 및 요청에 따라 비용이 발생할 수 있음 |
</aside>

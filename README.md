...
| 주요 특징 | - 간단하고 빠른 배포  
  - 자동화된 CI/CD  
  - Next.js, React 등 프레임워크 지원 | - 높은 자유도와 설정 가능성  
  - Nginx 웹 서버를 활용한 성능 최적화  
  - EC2 인스턴스 관리 필요  
  - Docker로 배포환경 구축 용이 | - CDN 서비스  
  - 정적 자산 전송에 특화  
  - 글로벌 배포 및 캐싱  
  - S3와 함께 사용 가능 (정적 웹사이트 배포에 특화되어 있지만 React의 경우 내부 라우팅 기술 O) |
| 장점 | - **사용 편의성**: 개발자가 인프라 관리에 신경 쓸 필요 없음  
  - **빠른 배포 속도**: 빌드 및 배포 과정 자동화  
  - **다양한 기능**: 미리보기 배포, A/B 테스팅 등 지원 | - **유연성**: 서버 환경을 자유롭게 구성 가능  
  - **성능**: Nginx를 활용한 웹 서버 최적화 가능  
  - **확장성**: EC2 인스턴스 확장을 통해 트래픽 증가에 대응 가능  
...
| 단점 | - Vercel 플랫폼에 종속  
  - 커스터마이징 제한  
  - WebSocket 지원 X  
  - 사용량에 따라 비용 발생 O | - 서버 구축, 설정, 관리 등 개발자 부담  
  - 초기 설정 및 최적화에 시간과 노력이 필요  

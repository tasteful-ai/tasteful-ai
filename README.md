...
| **장점** | • `switch` 패턴 매칭 (Preview)  • `record` 클래스 도입  • `"""`을 사용한 텍스트 블록 기능 제공 | **• 가상 스레드(Virtual Threads) 도입**  - OS 스레드 사용량을 줄이고, 수백만 개의 가상 스레드를 실행 가능  - 기존 Java 코드 수정 없이 사용 가능  **• 기본 문자 인코딩이 UTF-8로 변경** |
